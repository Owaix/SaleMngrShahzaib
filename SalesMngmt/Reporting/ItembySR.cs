...
{
    public partial class ItembySR : Form
    {
        ...
        public void customerledgers(int code)
        {
            ...
            cmbSalesPerson.SelectedValue = Vendorcode;
        ...
        private void Category_Load(object sender, EventArgs e)
        {
            ...
            FillCombo(cmbSalesPerson, Employe, "EmployeName", "ID", 0);
        }
        ...
        {
            //CategorysDataGridView.Rows.Clear();
            int Itemid = Convert.ToInt32(cmbSalesPerson.SelectedValue);
            if (Itemid > 0)
            {
                string Query = "Select i.IName , d.Qty , d.SalesPriceP , d.Amt , s.EmployeName , EDate from Sales_M m " +
                               "inner join tbl_Employee s ON s.ID = m.SID " +
                               "inner join Sales_D d ON m.RID = d.RID " +
                               "inner join Items i ON i.IID = d.IID " +
                               "where m.CompID = @CompID AND EDate >= @dtFrom AND EDate <= @dtTo AND m.SID = @SID order by i.IName";
                ...
                cmd.Parameters.Add("@dtTo", SqlDbType.DateTime).Value = dtTOdate.Value;
                cmd.Parameters.Add("@dtFrom", SqlDbType.DateTime).Value = dtFrom.Value;
                cmd.Parameters.Add("@SID", SqlDbType.Int).Value = (int)cmbSalesPerson.SelectedValue;
        ...
        public void customerledger(int code)
        {
            ...
            CategorysDataGridView.Rows.Clear();
            //CategorysDataGridView.Rows.Add(1, 2, 3, 4, 5, 6, 7);
            cmbSalesPerson.SelectedValue = Convert.ToInt32(code);            
            dtFrom.Value = DateTime.Now;
