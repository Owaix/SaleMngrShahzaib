...
namespace Lib.Reporting
{
    public class ReportsController
    {
        ...
        {
            string query = @"select convert(varchar(10), gl.GLDate,101) as gldate,concat(tt.Symbol, '-',gl.RID  ) as reference,
                             gl.RID,gl.TypeCode, sum(debit) as debit,sum(Credit) as credit   from GL gl 
                             left join RP tt on gl.TypeCode = tt.TypeCode
                             where  (CAST(gl.GLDate as Date) >= CAST(@StartDate as Date) and CAST(gl.GLDate as Date ) <= CAST(@EndDate as Date)) and AC_Code=@acCode  
                             group by  gl.TypeCode,concat(tt.Symbol, '-',gl.RID ),convert(varchar(10), gl.GLDate,101)
                            ,gl.RID order by convert(varchar(10), gl.GLDate,101)";
...
group by COA_D.AC_Code , COA_D.AC_Title;";
      ...
      where     (CAST(tbl_Order.OrderDate as Date) >= CAST(@reportStartDate  as Date) and CAST(tbl_Order.OrderDate as Date ) <= CAST(@reportEndDate as Date)) And tbl_Order.CompanyID = @CompanyID and tbl_OrderDetails.itemID=@IID
        ...
        {
            var _query = @"select Items.IName as name ,Items.IID, Sales_D.RID,ISNULL(sum(Sales_D.Qty),0) as Qty , ISNULL(sum(Sales_D.Qty),0) as Disc, 
